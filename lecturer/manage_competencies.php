<?php
require_once __DIR__ . '/../includes/auth_check.php';
require_role('lecturer');

$pdo = getPDO();
$user_id = $_SESSION['user_id'];
$errors = [];

// Get lecturer's assigned course IDs
$stmt = $pdo->prepare('SELECT course_id FROM lecturer_courses WHERE user_id = ?');
$stmt->execute([$user_id]);
$my_course_ids = array_column($stmt->fetchAll(), 'course_id');

if (empty($my_course_ids)) {
    set_flash('danger', 'You are not assigned to any courses. Please assign a course first.');
    redirect('/lecturer/manage_courses.php');
}

// Handle create / delete
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        $errors[] = 'Invalid request. Please try again.';
    } else {
        $action = $_POST['action'] ?? '';

        if ($action === 'create') {
            $course_id       = (int)($_POST['course_id'] ?? 0);
            $competency_code = trim($_POST['competency_code'] ?? '');
            $competency_name = trim($_POST['competency_name'] ?? '');
            $description     = trim($_POST['description'] ?? '');

            if (!in_array($course_id, $my_course_ids)) {
                $errors[] = 'You can only add competencies to your assigned courses.';
            }
            if ($competency_name === '') $errors[] = 'Competency name is required.';

            if (empty($errors)) {
                $stmt = $pdo->prepare('INSERT INTO competencies (course_id, competency_code, competency_name, description) VALUES (?, ?, ?, ?)');
                $stmt->execute([$course_id, $competency_code, $competency_name, $description]);
                log_activity('lecturer_create_competency', 'Created competency: ' . $competency_name);
                set_flash('success', 'Competency created successfully.');
                redirect('/lecturer/manage_competencies.php');
            }
        } elseif ($action === 'delete') {
            $comp_id = (int)($_POST['competency_id'] ?? 0);
            // Verify the competency belongs to one of the lecturer's courses
            $stmt = $pdo->prepare('SELECT course_id FROM competencies WHERE competency_id = ?');
            $stmt->execute([$comp_id]);
            $comp = $stmt->fetch();
            if ($comp && in_array($comp['course_id'], $my_course_ids)) {
                $stmt = $pdo->prepare('DELETE FROM competencies WHERE competency_id = ?');
                $stmt->execute([$comp_id]);
                log_activity('lecturer_delete_competency', 'Deleted competency ID: ' . $comp_id);
                set_flash('success', 'Competency deleted.');
            } else {
                set_flash('danger', 'Access denied.');
            }
            redirect('/lecturer/manage_competencies.php');
        }
    }
}

// Get competencies for the lecturer's courses
$placeholders = implode(',', array_fill(0, count($my_course_ids), '?'));
$stmt = $pdo->prepare("
    SELECT comp.*, co.course_code, co.course_name,
           (SELECT COUNT(*) FROM resources WHERE competency_id = comp.competency_id) AS res_count
    FROM competencies comp
    JOIN courses co ON co.course_id = comp.course_id
    WHERE comp.course_id IN ($placeholders)
    ORDER BY co.course_code, comp.competency_code
");
$stmt->execute($my_course_ids);
$competencies = $stmt->fetchAll();

// Get lecturer's courses for dropdown
$stmt = $pdo->prepare('
    SELECT c.course_id, c.course_code, c.course_name
    FROM courses c
    JOIN lecturer_courses lc ON lc.course_id = c.course_id
    WHERE lc.user_id = ?
    ORDER BY c.course_code
');
$stmt->execute([$user_id]);
$my_courses = $stmt->fetchAll();

$page_title = 'Manage Competencies';
require_once __DIR__ . '/../includes/header.php';
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="mb-0">Manage Competencies</h2>
    <a href="#create-form" class="btn btn-primary btn-sm">
        <i class="bi bi-plus-lg"></i> Add Competency
    </a>
</div>

<?php if ($errors): ?>
    <div class="alert alert-danger">
        <ul class="mb-0">
            <?php foreach ($errors as $err): ?>
                <li><?= e($err) ?></li>
            <?php endforeach; ?>
        </ul>
    </div>
<?php endif; ?>

<!-- Create Form -->
<div class="card shadow-sm mb-4" id="create-form">
    <div class="card-header">
        <h5 class="mb-0"><i class="bi bi-plus-circle me-2"></i>Create New Competency</h5>
    </div>
    <div class="card-body">
        <form method="POST">
            <input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>">
            <input type="hidden" name="action" value="create">

            <div class="row g-3">
                <div class="col-md-4">
                    <label class="form-label">Course <span class="text-danger">*</span></label>
                    <select name="course_id" class="form-select" required>
                        <option value="">-- Select Course --</option>
                        <?php foreach ($my_courses as $co): ?>
                            <option value="<?= (int)$co['course_id'] ?>"
                                <?= ($_POST['course_id'] ?? '') == $co['course_id'] ? 'selected' : '' ?>>
                                <?= e($co['course_code']) ?> – <?= e($co['course_name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-2">
                    <label class="form-label">Code</label>
                    <input type="text" name="competency_code" class="form-control"
                           placeholder="e.g. DB-01"
                           value="<?= e($_POST['competency_code'] ?? '') ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Competency Name <span class="text-danger">*</span></label>
                    <input type="text" name="competency_name" class="form-control"
                           value="<?= e($_POST['competency_name'] ?? '') ?>" required>
                </div>
            </div>
            <div class="row g-3 mt-1">
                <div class="col-md-12">
                    <label class="form-label">Description</label>
                    <input type="text" name="description" class="form-control"
                           value="<?= e($_POST['description'] ?? '') ?>">
                </div>
            </div>

            <div class="mt-3">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-lg me-1"></i> Create Competency
                </button>
            </div>
        </form>
    </div>
</div>

<!-- Competencies Table -->
<h5 class="mb-3">Your Course Competencies</h5>
<?php if (empty($competencies)): ?>
    <div class="alert alert-info">No competencies found for your courses.</div>
<?php else: ?>
    <div class="table-responsive">
        <table class="table table-hover align-middle">
            <thead class="table-light">
                <tr>
                    <th>Course</th>
                    <th>Code</th>
                    <th>Competency Name</th>
                    <th>Description</th>
                    <th>Resources</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($competencies as $comp): ?>
                    <tr>
                        <td><span class="badge bg-primary"><?= e($comp['course_code']) ?></span></td>
                        <td><?= e($comp['competency_code'] ?: '–') ?></td>
                        <td><?= e($comp['competency_name']) ?></td>
                        <td class="text-muted small"><?= e(mb_strimwidth($comp['description'] ?? '', 0, 50, '...')) ?></td>
                        <td class="text-center"><?= (int)$comp['res_count'] ?></td>
                        <td class="text-end">
                            <form method="POST" class="d-inline"
                                  onsubmit="return confirm('Delete this competency and all its resources?')">
                                <input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>">
                                <input type="hidden" name="action" value="delete">
                                <input type="hidden" name="competency_id" value="<?= (int)$comp['competency_id'] ?>">
                                <button type="submit" class="btn btn-sm btn-outline-danger">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
<?php endif; ?>

<?php require_once __DIR__ . '/../includes/footer.php'; ?>
